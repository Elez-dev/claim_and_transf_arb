...
def transfer(web3, privatekey, transfer_address):
    adr_token = Web3.toChecksumAddress('0x912CE59144191C1204E64559FE8253a0e49E6548')
    ...
    account = web3.eth.account.privateKeyToAccount(privatekey)
    address_wallet = account.address
    contract = web3.eth.contract(address=adr_token, abi=ABI_token)
    count_ = 0
    while True:
        ...
        if amount > 0:
            ...
            print(f'Количество токенов - {amount}')
        if count_ > 30:
...
def main():
    while keys_list:
        ...
        while True:
            if w3.eth.block_number < 16890400:
                print('Клейм токенов еще не начался')
            else:
                break
        RPC = YOUR_RPC
        privatekey = keys_list.pop(0)
        addrs = address_list.pop(0)
        web3 = Web3(Web3.HTTPProvider(RPC))
        count = 0
        while True:
            res = mint(web3, privatekey)
            count += 1
            if res is True:
                break
            if count > 30:
                return 0
        time.sleep(1)
        transfer(web3, privatekey, addrs)
...
if __name__ == '__main__':
    ...
    with open("address.txt", "r") as f:
        address_list = [row.strip() for row in f]
